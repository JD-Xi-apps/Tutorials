...
| Field | Value |
|---|---|
| Tutorial | **B06 — Change the sound with knobs** (beginner, order 6, 10 steps, ~10 min) |
...
The master plan (§8) keeps B06 **shallow and ear-first**: Cutoff, Resonance, the AMP/ENV
Envelope knob, LFO Rate and Depth, dramatic audible changes, and a safe way back. It defers
waveform architecture, Sub OSC, Pulse Width and *formal filter/envelope/LFO theory*. The
reconciliation brief adds that B06 must not be centred on filter taxonomy or deep synthesis
structure.

Two changes followed.

**The Level step was removed** (old position 6, *Set the part's own level*). It was
correctly sourced — OM p.8 for the AMP `[Level]` knob, OM p.2 item 7 for Master Volume —
but Level is not a sound-shaping control, and it is not in the master plan's list for this
tutorial. Its real subject was *balancing parts against each other*, which only becomes a
live question once there are several parts playing at once. Nothing is orphaned: the
control is described on demand in the Hardware Explorer, and balance is met where it
matters, when N06 auditions combinations and I07 assembles four parts.

**The filter-type step lost its taxonomy.** It previously told the learner that "the digital
parts offer four types; the Analog Synth part has only a low-pass filter", and the evidence
row quoted Roland's LPF / HPF / BPF / PKG list. That is precisely the filter taxonomy the
brief says B06 must not be centred on. The step survives, because *why does Cutoff sometimes
do the opposite of what I expected* is a real beginner question with a one-button answer —
but it now says only that the button changes what Cutoff takes away, and asks the learner to
listen. The Analog difference is stated as "fewer options", which is true and sufficient,
rather than as a named-type list. The full list belongs to the Hardware Explorer and to
`I04`, the comparative filter lab.

<!-- removed-steps:begin -->

Ids that no longer exist in B06: `B06-S11`. Removing old position 6 closed the numbering up
by one, so every step from *Change the shape of the note* onward moved down an id.
...
B06-S10's wording repeats Roland's scope ("after you have switched or edited the
