...
| Field | Value |
|---|---|
| Tutorial | **N04 — Make a simple drum beat** (novice, order 4, 13 steps, ~14 min) |
...
The master plan (§9) lists what N04 owns. Three items were missing, and all three are now
present:

| Master-plan requirement | Before | Now |
|---|---|---|
| choose a usable kit | **absent** — the tutorial used whatever kit was loaded | `N04-S03` |
| identify kick, snare **and closed hi-hat** | kick and snare only | `N04-S04`, `N04-S07`, `N04-S09` |
| one small actual drum-sound edit | **absent** | `N04-S10` |

The kit step matters more than it looks. Without it the learner built a beat on whatever kit
the loaded program happened to carry, which meant the tutorial's one aesthetic decision —
what the beat sounds like — was made for them. The hi-hat completes the three-instrument set
almost every drum pattern is built from, so the tutorial now finishes with a beat rather than
a demonstration.

The drum-sound edit is deliberately the *smallest* one available: a single panel knob the
learner already met in `B06`, acting on the part rather than on an instrument. Deep drum-kit
editing is explicitly deferred by the master plan, and PG's per-partial architecture (26
partials of WAVE, TVF and TVA, PG p.7) is not mentioned anywhere.

No step was removed. N04 went from 10 steps to 13, and the estimate from 12 minutes to 14.

## Sources consulted

| Source | Where it is used |
|---|---|
| Owner's Manual `eng07` | **p.5** *Choosing a Tone — Drums part*; **p.8** FILTER applies to the Digital Synth/Drums part;  **p.5** *Choosing a Tone* → Drums part; the drum instrument names printed above the keys and the `BD1` example; **p.10** *Deleting All Notes at a Specific Step*; **p.11** *What is TR-REC?*, the TR-REC procedure for drums, and the note that the [01]–[16] buttons light or go dark to show the played instrument's steps; **p.12** *Saving a Pattern*; **p.17** the Drums high-register entry |
...
| **N04-S03** Choose a drum kit | `toneButtons` | `full-plus-inset` | On the Drums part the Tone buttons select a drum kit rather than a single sound. | OM p.5 *Choosing a Tone — Drums part*: "Use the Part Select buttons to select Drums. Use the Tone [-] [+] buttons to select a tone (drum kit)." Roland's own parenthesis is what licenses calling it a kit. That every kit has a bass drum, snare and hi-hats is supported by the *Preset Drum Kit Waveform Assign List* (PG p.48), which assigns instruments per note across the kits; the step states it as a general expectation rather than naming any kit. |
| **N04-S04** Choose a drum sound | `drumsButton`, `keys` | `full` | Every key is a different instrument, named above the keys; the key you press last is the instrument you are about to record. | OM p.11: "If the Drums part is selected, press a note on the keyboard to specify the instrument that you want to record." OM p.5: "The Drums part lets you play a different instrument on each key. The instrument name is printed above each key; for example 'BD1.'" The `BD1` reference in `recoveryHelp` is Roland's own example. The silent high register is OM p.17. |
| **N04-S05** Light your first steps | `stepButton01`, `stepButton05`, `stepButton09`, `stepButton13` | `full-plus-inset` | Pressing a numbered button lights it, meaning the instrument sounds at that step; pressing a lit one turns it off. | OM p.11 TR-REC step 2: "Press the [01]–[16] buttons to make the button light for each step at which you want the instrument to sound. To erase a note, press the corresponding [01]–[16] button to make it go dark." That 01/05/09/13 are evenly spaced across sixteen is arithmetic, and Roland's own figure shows a bass drum on exactly that spacing. |
| **N04-S06** Listen to your beat | `playStopButton` | `full-plus-inset` | Play/Stop plays the pattern; anything already in it plays too. | OM p.11 TR-REC step 3. |
| **N04-S07** Choose a second drum sound | `keys`, `stepButtons` | `full` | Playing a different key changes which instrument the row is showing; the row is a view of one instrument's steps. You can do this during playback. | OM p.11: "For the Drum part, playing an instrument on the keyboard makes the [01]–[16] buttons light or go dark to indicate the steps on which that instrument will sound", and "You can change the on/off status of the [01]–[16] buttons even during playback." Also OM p.11: "If you select the Drums part, you'll record separately for each instrument that's assigned to each note of the keyboard." |
| **N04-S08** Add the second sound | `stepButton05`, `stepButton13` | `full-plus-inset` | Steps can be added while the pattern plays. | OM p.11, same sentence about changing status during playback. The musical description ("lands between the pulses") is a listening cue, not a Roland claim. |
| **N04-S09** Add a closed hi-hat | `keys`, `stepButtons` | `full-plus-inset` | A short tight metallic sound higher up the keyboard is a closed hi-hat; instrument names are printed above the keys; hi-hats often come in a closed and an open form. | OM p.5: "The Drums part lets you play a different instrument on each key. The instrument name is printed above each key; for example 'BD1.'" The closed/open pairing is visible in Roland's own naming across the *Preset Drum Kit Waveform Assign List* (PG p.48). **No key number is given** — the step says "higher up" and tells the learner to listen and read the panel, because instrument placement varies by kit and this tutorial must not assert a fixed key. |
| **N04-S10** Change the sound of the kit | `cutoffKnob` | `full-plus-inset` | Cutoff works on the Drums part; it adjusts the kit loaded into the part, so the whole beat changes together. | OM p.8 heads the filter's own description *Digital Synth/Drums part*, so the FILTER section demonstrably applies here. That it acts on the whole kit rather than one instrument is PG p.11's part parameter `Cutoff Ofst`: "Adjusts the cutoff frequency for the tone/drum kit that's assigned to the part." The step claims nothing about per-instrument filtering, and its recovery attributes a no-change result to the wrong part being selected rather than to any per-instrument behaviour. |
| **N04-S11** Silencing a step is not erasing it | `stepButtons` | `full-plus-inset` | Turning a lit button dark only mutes the note; turning it back on restores it. | OM p.10 *Deleting All Notes at a Specific Step*, quoted closely: "turning off a button that contains a note (making the button go dark) prevents that note from sounding. This only mutes the note and does not delete it; if you turn on the button once again (making the button light), its note resumes sounding." PG p.3 repeats it. |
| **N04-S12** Actually erasing a step | `eraseButton`, `stepButtons` | `full-plus-inset` | Hold Erase and press a step to completely erase its notes; the pattern must be stopped. Needed because TR-REC will not overwrite an existing note. | OM p.10: "If you want to completely erase the notes of a step, stop the pattern, hold down the [Erase] button, and press the button of the step that you want to erase." OM p.11 / PG p.3: "If you want to use TR-REC to re-input notes at a step in which you previously input a note… you must first delete the existing notes." |
| **N04-S13** Your beat is not saved yet | `display` | `full-plus-inset` | A pattern lives in the loaded program; changing program or powering off loses it; there is no separate pattern save. | OM p.12 *Saving a Pattern*, and its cross-reference to the program WRITE procedure. |
...
(N04-S10) is separate, explicit, and requires the pattern to be stopped.
...
(N04-S06's `recoveryHelp` covers hearing more than expected), which part or instrument
was selected on entry, or which steps are already lit.

One deliberate wording choice: N04-S04's checkpoint says the numbered buttons **may**
...
- **No step is named as musically correct.** N04-S08's `recoveryHelp` says outright that
