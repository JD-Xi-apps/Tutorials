...
/* Until a surface is implemented its route legitimately falls back to #home,
   which would otherwise pass silently. --strict-routes turns that fallback
   into a failure, so "the route exists" is asserted rather than assumed. */
...
(async () => {
    ...
    if (st.hash !== r) {
      ...
      if (strictRoutes) {
        ...
        results.failures.push({
          route: r,
          problems: [`fell back to "${st.hash}" - route not implemented`],
        });
      }
    }
    if (shotsDir) {
  ...
  if (results.redirects.length) {
    console.log(`routes falling back (not yet implemented): ${results.redirects.length}`);
